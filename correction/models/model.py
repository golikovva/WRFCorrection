...
class DETrajGRU(nn.Module):
    def __init__(self, encoders, forecaster, num_channels=2):
        super().__init__()
        self.encoders = nn.ModuleList(encoders)
        self.forecaster = forecaster

    def forward(self, input):
        channels = torch.split(input, 1, dim=-3)
        state = []
        for i, channel in enumerate(channels):
            # print(channel.shape)
            state.append(self.encoders[i](channel))
        # print(len(state), len(state[0]), type(state[0][0]))
        state = tuple(map(functools.partial(torch.cat, dim=-3), zip(*state)))
        # print(state[0].shape)
        output = self.forecaster(state)
        return output
