...
class WRFDataset(Dataset):
    def __init__(self, wrf_files, era_files, station_files=None, scatter_files=None,
        ...
        self.metadata['start_date'] = self.get_date_by_datefile_id(0, np_like=True)  # todo избавиться от залупной переменной
    ...
    def get_day_hour_encoding(self, i, frequency=1, debug=False):
        ...
        if debug:
            return day_encoded, hour_encoded, day, hour, d1, d2,
        return day_encoded, hour_encoded

    @staticmethod
    def get_time_encoding(date_normalized, lon, lat, frequency=1, periodic_law=np.sin):
    ...
    def load_stations(self, station_files):
        ...
        if stations.dtype == type(object):
            stations[:, :, 0] = stations[:, :, 0].astype('datetime64[s]').astype(float)
            stations = stations.astype(float)
        dates = stations[:, 0, 0]#.astype('datetime64[s]').astype(float)
        slices = np.where((dates == border_ts[0]) | (dates == border_ts[1]))[0]
    ...
    def get_data_by_id(self, i, file_attr, var_attr):
        ...
        while needed_len > 0:
            try:
                ...
                print(path_i, getattr(self, file_attr)[path_i], 'error ocured in this file')
                file_vars = self.load_file_vars(getattr(self, file_attr)[path_i+1], getattr(self, var_attr))
