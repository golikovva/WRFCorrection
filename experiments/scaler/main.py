...
for wrf_file, era_file in tqdm(zip(wrf_files, era_files), total=len(wrf_files)):
    wrf_tensor.append(torch.from_numpy(load_nc_vars(wrf_file, ['uvmet10', 'T2', 'LANDMASK', 'XLAT', 'XLONG'])))
    era_tensor.append(torch.from_numpy(load_nc_vars(era_file, ['u10', 'v10', 't2m'])))
