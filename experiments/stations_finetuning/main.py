...
folder_name = cfg.model_type + '_finetuned'
...
train_files, val_files, test_files = split_train_val_test(cfg.data.wrf_folder, cfg.data.era_folder, 0.7, 0.1, 0.2, cfg.data.start_date, cfg.data.end_date)
...
if 'train' in cfg.run_config.run_mode:
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=cfg.run_config.batch_size, shuffle=True,
                                  ...
                                  collate_fn=none_consistent_collate, pin_memory=True)
    ...
    valid_dataloader = DataLoader(valid_dataset, batch_size=cfg.run_config.batch_size, shuffle=False,
                                  num_workers=cfg.run_config.num_workers,
                                  collate_fn=none_consistent_collate)
...
meaner = MeanToERA5(os.path.join(cfg.data.logs_folder, 'wrferaMapping.npy'),
    ...
    .to(cfg.device)
stations_interpolator = InvDistTree(x=wrf_coords, q=metadata['coords'], device=cfg.device) if station_files else None
...
criterion = TurbulentMSE(meaner, betas, stations_interpolator, scat_interpolator, logger=logger,
                         kernel_type=cfg.loss_config.loss_kernel, k=cfg.loss_config.k, device=cfg.device).to(cfg.device)

model = build_correction_model(cfg)
model.unet.freeze_backbone()
model.unet.set_mode(unet_mode='eval', transformer_mode='train')
trained_model_name = f'/home/logs/BERTunet/misc_104/models/model_12.pth'
